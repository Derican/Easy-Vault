...
public class RaidChallengeGenerator extends VaultGenerator {
    ...
    public boolean generate(ServerWorld world, VaultRaid vault, BlockPos.Mutable pos) {
        MutableBoundingBox box = vault.getProperties().getBase(VaultRaid.BOUNDING_BOX).orElseGet(() -> {
            ...
            BlockPos max = pos.move(8192, 0, 0).immutable();
        ...
        try {
            ...
            StructureStart<?> start = ModFeatures.RAID_CHALLENGE_FEATURE.generate(jigsaw, world.registryAccess(),
            ...
            world.getChunk(chunkPos.x, chunkPos.z, ChunkStatus.EMPTY, true).setStartForFeature(ModStructures.RAID_CHALLENGE, start);
            tick(world, vault);
...
/* Location:              C:\Users\Grady\Desktop\the_vault-1.7.2p1.12.4.jar!\iskallia\vault\world\vault\gen\RaidChallengeGenerator.class
