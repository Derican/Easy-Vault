...
public class BossSpawnPieceProcessor extends VaultPieceProcessor {
    ...
    public void postProcess(final VaultRaid vault, final ServerWorld world, final VaultPiece piece, final Direction generatedDirection) {
        ...
        final BlockPos stabilizerPos = BlockPos.betweenClosedStream(piece.getBoundingBox()).map(pos -> new Tuple<>(pos, world.getBlockState(pos))).filter(tpl -> (tpl.getB()).getBlock() instanceof VaultLootableBlock && ((VaultLootableBlock) (tpl.getB()).getBlock()).getType() == VaultLootableBlock.Type.VAULT_OBJECTIVE).findFirst().map(Tuple::getA).orElse(null);
