...
public class RaidChallengeStructure extends Structure<RaidChallengeStructure.Config>
{
    ...
    public static class Start extends MarginedStructureStart<Config>
    {
        ...
        public void generate(final JigsawGenerator jigsaw, final DynamicRegistries registry, final ChunkGenerator gen, final TemplateManager manager) {
            ...
            jigsaw.generate(registry, new VaultStructure.Config(() -> registry.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY).get(Vault.id("raid/starts")), 1).toVillageConfig(), AbstractVillagePiece::new, gen, manager, this.pieces, (Random)this.random, false, false);
            this.calculateBoundingBox();
        }
    }
    ...
    public static class Pools
    {
        ...
        static {
            START = JigsawPatternRegistry.register(new JigsawPattern(Vault.id("raid/starts"), new ResourceLocation("empty"), (List)ImmutableList.of(Pair.of(JigsawPiece.single(Vault.sId("raid/starts"), ProcessorLists.EMPTY), 1)), JigsawPattern.PlacementBehaviour.RIGID));
        }
    }
}
